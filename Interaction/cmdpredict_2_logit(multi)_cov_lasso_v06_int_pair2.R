# PROJECT : PREDICTION OF CARDIOMETABOLIC CARDIOMETABOLIC DISEASE 
# DATE  : 20 APRIL 2025
...
## CONFIGURATION BLOCK
##
## 1. Set the 5 variables below for the interaction you want to test.
## 2. Run the entire script.
## 3. Repeat for each of your 5 pairs.
#################################################

# --- 5 PAIRS ---
# Pair 1: Age x sex
# interaction_name <- "int_pair1"
# var1 <- "age1"; var2 <- "sex1_2"; var1_type <- "continuous"; var2_type <- "categorical"
#
# Pair 2: Foreign-born x low-education
# interaction_name <- "int_pair2"
# var1 <- "born1_2"; var2 <- "education2_2"; var1_type <- "categorical"; var2_type <- "categorical"
#
# Pair 3: Sex x HDL
# interaction_name <- "int_pair3"
# var1 <- "sex1_2"; var2 <- "hdl"; var1_type <- "categorical"; var2_type <- "continuous"
#
# Pair 4: Smoking x non-HDL-C
# interaction_name <- "int_pair4"
# var1 <- "smoking1_2"; var2 <- "nonhdl"; var1_type <- "categorical"; var2_type <- "continuous"
#
# Pair 5: Waist circumference x non-HDL-C
# interaction_name <- "int_pair5"
# var1 <- "waist"; var2 <- "nonhdl"; var1_type <- "continuous"; var2_type <- "continuous"

# --- SET INTERACTION PAIR ---
interaction_name <- "int_pair2"
var1 <- "born1_2"
var2 <- "education2_2"
...
# --- Output file will be named automatically based on interaction_name ---
output_file <- interaction_name 
...
library(rlang) # Needed for dynamic variable names
...
input_data <- file.path(intermediate_data, "logit(multi)_cov_df_v06.rds")
input_fold <- file.path(intermediate_data, "logit(multi)_cov_fold_v06.rds")

# --- Dynamic output file name ---
output_result <- file.path(intermediate_data, paste0("logit(multi)_cov_lasso_cvresult_v06_", output_file, ".rds"))
...
# Add Cat x Cat interaction to category_col
if (var1_type == "categorical" && var2_type == "categorical") {
  category_col <- c(category_col, interaction_name)
  cat("Added", interaction_name, "to categorical predictor list.\n")
}
...
predictor_col <- c( 'time_days',
                    ...
                    'expense1_2',
                    'born1_2',
                    interaction_name, # <--DYNAMIC INTERACTION NAME
                    'location1_2',
                    'site1_2',
                    'site1_3',
                    'site1_4',
...
# Find new interaction term index for plotting
idx_interaction <- which(predictor_col == interaction_name)
...
# Add all key indices
key_beta_indices <- c(idx_age, idx_smoke, idx_sbp, idx_waist, idx_hdl, idx_interaction)
...
print(paste("Starting", k, "-Fold Cross-Validation..."))
print(paste("Running analysis for interaction:", interaction_name))
...
for (i in 1:k) {
  ...
  # --- [FIX] Create CATEGORICAL x CATEGORICAL interaction (if configured) ---
  # This block was moved here from before the loop.
  # It now correctly modifies df_train and df_test.
  if (var1_type == "categorical" && var2_type == "categorical") {
    df_train <- df_train %>%
      mutate(!!interaction_name := !!sym(var1) * !!sym(var2))
    df_test <- df_test %>%
      mutate(!!interaction_name := !!sym(var1) * !!sym(var2))
    cat("Created categorical interaction term:", interaction_name, "\n")
  }
  ...
  # --- Create CONTINUOUS x (Cont/Cat) interaction term ---
  # This MUST be done AFTER standardization.
  if (var1_type == "continuous" || var2_type == "continuous") {
    ...
    cat("Created standardized interaction:", interaction_name, "\n")
  }
  ...
  # DESIGN_train will now automatically include the correct interaction column
  ...
  stan_data_fold <- list(
    N = nrow(DESIGN_train),
    D = ncol(DESIGN_train), # D will be correct for this model
    DESIGN = DESIGN_train,
  ...
  # ... (extraction, summary, saving results) ...
  ...
  # This will now include the dynamic interaction term
