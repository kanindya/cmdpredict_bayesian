...
input <- file.path(intermediate_data, "logit(multi)_m8_lasso_cvresult_v06.rds")
...
#################################################
# 0) Sanity: ensure probabilities (if logits, convert)
p <- all_pred$predicted_value

# 1) Ground truth as 0/1 (robust to factor level order)
y <- as.integer(all_pred$true_outcome_factor == "1")

# 2) Sanity checks
stopifnot(all(p >= 0 & p <= 1))                        # must be probabilities
y_alt <- as.numeric(as.character(all_pred$true_outcome_factor))
stopifnot(all(y %in% c(0,1)), all(y_alt %in% c(0,1)))  # both encodings are 0/1
stopifnot(all(y == y_alt))                              # they match exactly

# 3) Brier score (MSE of probs)
mse_value <- mean((y - p)^2)
cat("Brier (MSE of probabilities):", format(mse_value, digits = 6), "\n")

# 4) Brier Skill Score (vs prevalence baseline computed on SAME y)
pi_hat <- mean(y)
brier_ref <- pi_hat * (1 - pi_hat)
bss <- 1 - mse_value / brier_ref
cat("Brier Skill Score:", format(bss, digits = 6), "\n")
...
if (!is.null(roc_obj) && inherits(roc_obj, "roc")) {
  ...
  roc_plot <- ggroc(roc_obj, colour = 'darkblue', size = 1) +
    ...
    coord_fixed()
  print(roc_plot)
  roc_plot_filename <- "07. Graph/03. Paper 3/logit(multi)_m8_lasso_ROC_v06.png"
...
ggsave(filename = "07. Graph/03. Paper 3/logit(multi)_m8_lasso(1)_v06.png",
       plot = cal_plot,
...
# Controls
x_max    <- 0.4      # axis cap (change to 1 if you want 0–1 like the example)
y_max    <- 0.4
hist_frac <- 0.15        # fraction of y-axis height used by the bars (0–1)
binwidth  <- 0.02        # histogram bin width

n_total <- nrow(cal_dat)
p_mean  <- mean(cal_dat$p)
p_median<- median(cal_dat$p)


# ---- colours ----
cal_blue <- "#063970"   # pick any blue you like

# ---- fit logistic recalibration and build 95% CI grid (0–0.40) ----
if (!"lp" %in% names(cal_dat)) cal_dat$lp <- qlogis(cal_dat$p)
fit_recal <- glm(y ~ lp, family = binomial(), data = cal_dat)

p_grid <- seq(1e-6, x_max, length.out = 400)
grid   <- data.frame(p = p_grid, lp = qlogis(p_grid))
pr     <- predict(fit_recal, newdata = grid, type = "link", se.fit = TRUE)
grid$phat <- plogis(pr$fit)
grid$lwr  <- plogis(pr$fit - 1.96 * pr$se.fit)  # 95% band
...
cal_plot <- ggplot() +
  # frequency bars
  geom_histogram(
    data = subset(cal_dat, p <= x_max),
    aes(x = p, y = after_stat(ncount) * (hist_frac * y_max)),
    binwidth = binwidth, boundary = 0, closed = "left",
    fill = "grey70", color = "grey60", alpha = 0.8
  ) +
  # 95% band around the recalibration curve
  geom_ribbon(data = grid, aes(p, ymin = lwr, ymax = upr),
              fill = cal_blue, alpha = 0.15) +
  
  # 45° reference
  geom_abline(intercept = 0, slope = 1, linetype = "dashed",
              color = "grey55", linewidth = 0.5) +
  
  # binned observed rates (now blue points + blue CI)
...
# Save
ggsave("07. Graph/03. Paper 3/logit(multi)_m8_lasso(2)_v06.png",
