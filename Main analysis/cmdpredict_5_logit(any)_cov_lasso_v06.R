# PROJECT : PREDICTION OF ANY CARDIOMETABOLIC DISEASE
# DATE    : 27 APRIL 2025
...
f_pred_rds <- file.path(inter_data, "logit(any)_cov_pred_lasso_v05.rds")
f_prob <- file.path(inter_data, "logit(any)_cov_prob_lasso_v05.rds") 
f_stanfit <- file.path(inter_data, "logit(any)_full_stanfit_v05.rds")
f_inputs <- file.path(inter_data, "logit(any)_full_stan_inputs_v05.rds")

# Output files for Step 5
f_subgrp_rds <- file.path(results_dir, "logit(any)_subgroup_pred_v05.rds") 
f_subgrp_csv <- file.path(excel_dir, "logit(any)_subgroup_pred_v05.csv") 
f_or_csv <- file.path(excel_dir, "logit(any)_or_v05.csv") 
...
# --- CORRECTED Reference Group Definitions for BINARY HDL/Waist ---
