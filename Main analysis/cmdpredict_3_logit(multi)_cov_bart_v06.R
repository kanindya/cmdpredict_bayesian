...
input <- file.path(intermediate_data, "logit(multi)_cov_bart_v05.rds") 
...
#################################################
## Stan-Specific Diagnostic Section - REMOVE OR COMMENT OUT FOR BART
#################################################
# print("--- Convergence Summary Across Folds ---")
# all_max_rhats <- sapply(cv_results, function(res) res$max_rhat) # These fields won't exist for BART results
...
if (!is.null(roc_obj_bart) && inherits(roc_obj_bart, "roc") && !is.na(auc_value_bart)) {
  ...
  roc_plot_bart <- ggroc(roc_obj_bart, colour = 'forestgreen', size = 1) + # Changed color
    ...
    coord_fixed()
  print(roc_plot_bart)
  roc_plot_filename_bart <- "07. Graph/03. Paper 3/logit(multi)_cov_bart_v05.png" # New filename
...
ggsave(filename = "07. Graph/03. Paper 3/logit(multi)_cov_bart(1)_v06.png",
       plot = cal_plot,
...
# Save
ggsave("07. Graph/03. Paper 3/logit(multi)_cov_bart(2)_v06.png",
