...
# DATE  : 14 APRIL 2025
# NOTES : STEP 3: Ccomple results (logit, no covariates)
...
# --- Define Input File ---
  ...
  input <- file.path(intermediate_data, "logit(multi)_cov_cvresult_v06.rds") # From Script 2
...
# Calculate summary
  or <- all_or %>%
    ...
    arrange(Predictor) %>%
    select(Predictor, median_OR, min_OR, max_OR, median_OR_LCI, median_OR_UCI, n_folds) # Optional: select columns
  
# Print or
  print("--- Summary of OR Distribution (Custom Order) ---")
  print(as.data.frame(or))
...
ggsave(filename = "07. Graph/03. Paper 3/logit(multi)_cov(1)_v06.png",
       plot = cal_plot,
...
# Save
ggsave("07. Graph/03. Paper 3/logit(multi)_cov(2)_v06.png",
