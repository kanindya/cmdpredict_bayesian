...
# Directory for intermediate data
  ...
  output_result <- file.path(intermediate_data, "logit(any)_cov_cvresult_v06.rds")
  stan_model <- "03. Do file/03. Paper 3/02. R/01. Stan/cmdpredict_single_logit_completed_v01.stan"
...
# --- List to store results ---
  ...
  for (i in 1:k) {
    ...
    # --- Run Stan Model ---
      ...
      fit <- stan(file = stan_model,      # Use the correct variable name
                  data = stan_data_fold,
                  chains = 4,             # Standard practice
                  iter = 2000,            # Recommend 2000 for better convergence (1000 warmup, 1000 sampling)
                  warmup = 1000,          # Explicitly define warmup
                  cores = 2, # Use defined cores
                  seed = 456 + i,         # Per-fold seed
                  control = list(adapt_delta = 0.85) # Optional: helps with divergent transitions if they occur
      )
    ...
    # --- Extract predictions ---
      pred_param <- "pred" # Correct name from Stan generated quantities
